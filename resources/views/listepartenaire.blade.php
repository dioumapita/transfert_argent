    ...
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-9" style="display: flex; justify-content: right;">
          <h1>ESPACE D'ADMINISTRATION DES PARTENAIRES SADIBO TRANSFERT</h1>
        </div>
        <div class="col-sm-3">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a class="btn btn-primary" href="{{route('accueil')}}">Accueil</a></li>
            <li class="breadcrumb-item active"><a class="btn btn-primary" href="{{route('creationpartenaire')}}">Ajouter un partenaire</a></li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
          ...
          <table id="example1" class="table table-bordered table-hover">
           <thead>
            ...
            <tbody>
              ...
              <tr>
              <th scope="row">{{$partenaire->nom_partenaire}}</th>
              <td>{{$partenaire->prenom_partenaire}}</td>
              <td>{{$partenaire->email_partenaire}}</td>
              <td>{{$partenaire->fonction_partenaire}}</td>
              <td>{{$partenaire->adresse_partenaire}}</td>
              <td>{{$partenaire->created_at}}</td>
              <td>
                <a class="btn btn-primary" href="{{route('edite_partenaire',$partenaire->id)}}">Edite</a>
                <a href="#" class="btn btn-danger" onclick="if(confirm('Vouler vous vraiment supprimer cet element?')){document.getElementById('form-{{$partenaire->id}}').submit()}">Supprimer</a>
                <form id="form-{{$partenaire->id}}" action="{{route('partenaire.supprimer',['partenaire'=>$partenaire->id])}}" method="post">
                  @csrf
